      ...
      <div className="max-w-[1400px] mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <h1 className="mt-2 font-display text-4xl md:text-5xl text-offwhite tracking-[2px]">
              Gestion de <span className="text-gold">Ordenes</span>
            </h1>
          </div>
              ...
              <button
                ...
                className={`text-[10px] font-mono uppercase tracking-[1px] px-3 py-1.5 rounded-sm border transition-all ${
                  filter === s
                    ...
                    : "border-border text-offwhite/30 hover:text-offwhite/50"
                }`}
              >
                {s || "Todas"}
              </button>
        ...
        {loading ? (
          ...
          <div className="p-12 bg-surface-light border border-border rounded-sm text-center">
            ...
            <p className="text-offwhite/40 text-sm">No hay ordenes {filter ? `con estado "${filter}"` : "aun"}.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {filtered.map((order) => (
              <div
                key={order.id}
                className="bg-surface-light border border-border rounded-sm overflow-hidden"
              >
                <button
                  onClick={() => setExpanded(expanded === order.id ? null : order.id)}
                  className="w-full p-5 flex items-center justify-between text-left hover:bg-surface/30 transition-colors"
                >
                  <div className="flex items-center gap-6 flex-wrap">
                    <div>
                      <p className="text-offwhite/80 text-sm font-medium">{order.customer_name}</p>
                    ...
                    <span className="text-sm text-offwhite/70 font-display tracking-[1px]">
                  ...
                  <div className="flex items-center gap-3">
                    {nextStatus(order.status) && (
                      <button
                        ...
                        className="text-[10px] font-mono uppercase tracking-[1px] px-3 py-1.5 bg-gold/10 text-gold border border-gold/20 rounded-sm hover:bg-gold/20 disabled:opacity-30 transition-all whitespace-nowrap"
                      >
                    ...
                    <span className="text-offwhite/20 text-sm">
                      {expanded === order.id ? "▲" : "▼"}
                    </span>
                  </div>
                </button>
                ...
                {expanded === order.id && (
                  <div className="px-5 pb-5 pt-0 border-t border-border/50">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 mb-4">
                      <div>
                        <p className="text-[10px] font-mono uppercase tracking-[1.5px] text-offwhite/30 mb-1">
                          Telefono
                        </p>
                        <p className="text-sm text-offwhite/70">{order.customer_phone}</p>
                      </div>
                      <div className="md:col-span-2">
                        <p className="text-[10px] font-mono uppercase tracking-[1.5px] text-offwhite/30 mb-1">
                          Direccion
                        </p>
                        <p className="text-sm text-offwhite/70">{order.customer_address}</p>
                      </div>
                    </div>
                        ...
                        <tbody>
                          {order.items.map((item) => (
                            <tr key={item.id} className="border-b border-border/30 last:border-0">
                              <td className="p-3 text-offwhite/70">{item.product_name}</td>
                              <td className="p-3">
                                <span className="text-[10px] font-mono uppercase tracking-[1px] px-2 py-0.5 bg-surface border border-border rounded-sm text-offwhite/50">
                                  {item.variant_label || "—"}
                                </span>
                              </td>
                              <td className="p-3 text-offwhite/70">{item.quantity}</td>
                              <td className="p-3 text-offwhite/70">${Number(item.price).toFixed(2)}</td>
                              <td className="p-3 text-offwhite/80 text-right">
